...
void InitializingNewTuple(Tuple& newTuple, Tuple oldTuple)
{
	...
	myiterator = field_names.begin();
	while(myiterator != field_names.end())
	{
...
Schema GetNewSchema(Relation*& relation1, Relation*& relation2)
{
	...
	vector<string>::iterator myiterator;
	myiterator = relation1_field_names.begin();
	while(myiterator != relation1_field_names.end())
	{
		string temp;
	...
	myiterator = relation2_field_names.begin();
	while(myiterator != relation2_field_names.end())
	{
		string temp1;
		if((*myiterator).find('.') == string::npos)
...
vector<Tuple> TScan::getUniqueTuples(string table_name, int index_memblock_end)
{
	...
	int index=0;
	while(index<num_blocks) //For all the blocks of Relation
	{
		relation_pointer->getBlock(index, index_memblock_end);
		vector<Tuple> t;
...
vector<Tuple> TScan::Ordering_Relation(vector<Tuple> tup, string table_name, vector<MyColumn> order_by)
{
		...
		vector<MyColumn>::iterator myiterator = order_by.begin();
		while(myiterator!=order_by.end()){
...
vector<Tuple> TScan::GetUniqueTuplesCorrected(bool isUnique, ProjectOP project_op, SelectOP& select_op, string table_name, int index_memblock_end, string temp_relation, vector<MyColumn> order_by)
{
		...
		while(index<num_blocks)
		{
			...
			vector<Tuple> tup;
			tup = block_pointer->getTuples();
			vector<Tuple>::iterator tupleiterator = tup.begin();
			while(tupleiterator != tup.end())
			{
				Tuple tup = *tupleiterator;
				if(select_op.Issearch_cond())
				{
		...
		if(isUnique)
		{
				...
				if(memory_block_size < NUM_OF_BLOCKS_IN_MEMORY)
				{
					...
					vector<Tuple>::iterator myiterator1 = tuples_displayed.begin();
					while(myiterator1 != tuples_displayed.end())
					{
		...
		//perform the project_op
			...
			Relation* newRelation_pointer = schema_manager.createRelation(temp_relation, schema_new);
			vector<Tuple>::iterator myiterator = tuples_displayed.begin();
			tuples_buffered.clear();
			while(myiterator != tuples_displayed.end())
			{
...
vector<Tuple> TScan::GetSelectTuples(string table_name, int index_memblock_end, SelectOP& select_op)
{
	...
	vector<Tuple>::iterator myiterator = myTuples.begin();
	while(myiterator!=myTuples.end())
	{
		Tuple tup = *myiterator;
		Mapping_Tuple(tup, table_name);
...
vector<Tuple> TScan::GetSelectTuples(bool isUnique, string table_name, int index_memblock_end, SelectOP& select_op, vector<MyColumn> order_by)
{
	...
	if(isUnique)
	{
		...
		else
		{
			...
			vector<Tuple>::iterator myiterator = output_tuple.begin();
			while(myiterator!=output_tuple.end())
			{
...
Tuple JoinTuples(Tuple tuple_left, Tuple tuple_right, Relation*& new_relationPointer, Relation*& left_relationPointer, Relation*& right_relationPointer)
{
	...
	vector<string>::iterator myiterator = left_field_names.begin();
	
	while(myiterator!= left_field_names.end())
	{
		string column_name;
		if(IsLeft)
	...
	myiterator = right_field_names.begin();
	while(myiterator!= right_field_names.end())
	{
		string column_name;
		if(IsRight)
			column_name = *myiterator;
...
vector<Tuple> TScan::GetProd(string tbl1, string tbl2, string tempTable, bool saveToDB)
{
	...
	while(1)
	{
		...
		while(blocks_to_read > 0) //while all blocks for the relation are not read
		{
			...
			vector<Tuple>::iterator big_myiterator = bigger_rel_tuples.begin();
			while(big_myiterator!= bigger_rel_tuples.end())
			{
				small_myiterator = smaller_rel_tuples.begin();
				while(small_myiterator!= smaller_rel_tuples.end())
				{
...
void SaveTupleTodisk(Relation*& relation_pointer, vector<Tuple> tuples, int memory_index)
{
	...
	vector<Tuple>::iterator myiterator = tuples.begin();
	while(myiterator!=tuples.end())
	{
...
Schema GetNewSchema(ProjectOP project_op, JOINOP join_op, Relation*& relation_pointer_old)
{
	...
	vector<string>::iterator table_iterator = mytables.begin();
	...
	myiterator = start_iterator;
	while(myiterator != end_iterator)
	{
					...
					SendRelation sendRelation, bool saved)
{
	...
	vector<Tuple>::iterator tuple_iterator = tuples.begin();

	while(tuple_iterator != tuples.end())
	{
...
string TScan::Fetch_Table_Name(string column_name)
{
	...
	vector<string>::iterator table_iterator = VariablesFromTbl.begin();

	vector<Schema> myschema;
	int count = 0;
	string table_name;
	while(table_iterator != VariablesFromTbl.end())
	{
