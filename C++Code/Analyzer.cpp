...
void Analyzer::OutputDiskIO(ofstream& qout_file, int out_option)
{
	...
	if(out_option == 1) {
		cout<<"No. of Disk I/O: "<<disk.getDiskIOs()<<"   Time elapsed in milliseconds: "<<disk.getDiskTimer()<<endl;
		cout<<"*****************************************************"<<endl;
		cout<<endl;
	}
	else if(out_option == 2) {
		qout_file<<"No. of Disk I/O: "<<disk.getDiskIOs()<<"   Time elapsed in milliseconds: "<<disk.getDiskTimer()<<endl;
		qout_file<<"************************************************"<<endl;
		qout_file<<endl;
	}
	else {}
...
{
	 cout<<"Parser is being started"<<endl;
}

Analyzer::Analyzer(char* input_text):lexer(input_text)
{
	if(!logoutput.is_open())
	{
		...
		if (!logoutput)
		{
  			cerr << "The file Database_Log.txt can not be opened" << endl;
  		}
  	}
  	else
  	{
...
//DELETE FROM table-name [WHERE search-condition]
{
	...
	//Get the next token
  	SqlToken mytoken = lexer.GetNextToken();
