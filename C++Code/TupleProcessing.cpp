...
bool Tuple_Comparison::operator()(const Tuple& tuple1, const Tuple& tuple2)
{
...
//count of blocks to sort in main memory
	...
	if(!success)
		cout<<"Error has been encountered in InMemorySort function"<<endl;
	return success;
}
...
bool IsEqualTuples(Tuple tup1, Tuple tup2, vector<string> fnames)
{
	...
	vector<string>::iterator myiterator;
	myiterator = fnames.begin();
	while(myiterator!=fnames.end())
	{
		if(schemat1.getFieldType(*myiterator) == INT)
		{
			if(tup1.getField(*myiterator).integer != tup2.getField(*myiterator).integer)
...
void Mapping_Tuple(Tuple map_tuple)
{
	...
	vector<string>::iterator myiterator;
	myiterator = myColmNames.begin();
	while(myiterator!=myColmNames.end())
	{
...
void Mapping_Tuple(Tuple map_tuple, string table_name)
{
	...
	vector<string>::iterator myiterator;
	myiterator = map_column_names.begin();
	
	while(myiterator!=map_column_names.end())
	{
