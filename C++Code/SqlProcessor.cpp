...
bool SqlProcessor::CreateTbl(string table_name, vector<string> attribute_namelist, vector<enum FIELD_TYPE> datatype_list, ofstream& qout_file, int out_option)
{
	...
	{
		if(out_option == 1) {
			cout<<"Table Creation has been FAILED!!!!"<<endl;
		}
		else if(out_option == 2) {
			qout_file<<"Table Creation has been FAILED!!!!"<<endl;
		}
		else {}
		return false;
	}
	if(out_option == 1) {
		cout<<"Table: "<<table_name<<"  has been created sucessfully"<<endl;
	}
	else if(out_option == 2) {
		qout_file<<"Table: "<<table_name<<" has been created sucessfully"<<endl;
	}
	else {}
	return true;
}
...
{
	bool pass = true;
	vector<string>::iterator attribute_iterator = attribute_list.begin();
	vector<string>::iterator value_iterator = value_list.begin();
	Schema scheme_of_tuple = tuple.getSchema();
	int index = 0;
	while(attribute_iterator != attribute_list.end())
	{
...
bool SqlProcessor::Insert_After_Select_Tbl(string table_name, vector<Tuple> tuple_list) {
	...
	Relation* relation = schema_manager.getRelation(table_name);
	if(relation == NULL) return false;
	vector<Tuple>::iterator iteration = tuple_list.begin();
	while(iteration != tuple_list.end())
	{
...
void Output(SendRelation myRelation, ofstream& qout_file, int out_option)
{
	...
	vector<Tuple>::iterator iteration = myRelation.tuples.begin();
	while(iteration != myRelation.tuples.end()) {
		if(out_option == 1) {
...
bool SqlProcessor::SelectTbl(LQP*& lp, ofstream& qout_file, int out_option)
{
...
SendRelation SqlProcessor::InsertSelectTbl(LQP*& lp)
{
	SendRelation retnrel = lp->Execute();
	// Output(retnrel);
	if(retnrel.marked_For_Delete)
...
bool SqlProcessor::DropTbl(string table_name, ofstream& qout_file, int out_option)
{
	...
	if(pass) {
		if(out_option == 1) {
			cout<<"Table has been dropped successfully: "<<table_name<<endl;
		}
		else if(out_option == 2) {
			qout_file<<"Table has been dropped successfully: "<<table_name<<endl;
		}
		else {}
	}
	return pass;
